...
BEiT-3/capemb) 1 LẦN, mở 1 API (FastAPI + ngrok) để backend LOCAL (aic-system/)
...
lạ đoán được URL ngrok rồi xài ké GPU miễn phí của bạn.
...
def _get_ngrok_token() -> str:
    """Kaggle Secret KHÔNG tự thành biến môi trường — phải đọc qua
    kaggle_secrets.UserSecretsClient() (giống cách 04_embed_dinov3.py đọc
    HF_TOKEN). Thiếu/quên bật Attach -> log cảnh báo rõ ràng, không chết ngầm."""
    ...
    try:
        ...
        print(f"  !!! CẢNH BÁO: không đọc được Kaggle Secret 'NGROK_AUTHTOKEN' "
              f"({type(e).__name__}: {e}) -> ngrok sẽ lỗi xác thực (ERR_NGROK_4018). "
              f"Vào Add-ons -> Secrets -> thêm 'NGROK_AUTHTOKEN' + bật Attach cho notebook này.",
              flush=True)
        return ""


_ngrok_token = _get_ngrok_token()
if _ngrok_token:
